...
这是一个 **Claude Code Skills（技能）** 仓库，存放自定义 skill 的定义文件和配套脚本。每个 skill 安装到 `~/.claude/skills/` 后，由 Claude Code 的 FleetView harness 加载，使 Claude 能够在对话中识别并执行特定任务。
...
正文为 skill 的执行指令，Claude 在 skill 被触发后会遵循这些指令工作。
...
### local-cc-digest
通过 `scripts/summarize_sessions.py` 扫描 `~/.claude/projects/` 的 JSONL session 文件，生成工作回顾报告。
